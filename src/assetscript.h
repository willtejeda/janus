...
public:
    ...
    QList<QPointer <RoomObject> > RunScriptCodeOnObjects(const QString & code, QHash <QString, QPointer <RoomObject> > & envobjects, QPointer <Player> player);
    QList<QPointer <RoomObject> > RunFunctionOnObjects(const QString & fn_name, QHash <QString, QPointer <RoomObject> > & envobjects, QPointer <Player> player, const QScriptValueList & args = QScriptValueList());
...
//    QList<QPointer <RoomObject> > OnCollision(QPointer<ObjectProperties> obj1, QPointer<ObjectProperties> obj2, QHash <QString, QPointer <RoomObject> > & objects);
    ...
    QList<QPointer <RoomObject> > OnKeyEvent(QString name, QKeyEvent * e, QHash <QString, QPointer <RoomObject> > & objects, QPointer <Player> player, bool * defaultPrevented);
    ...
    void UpdateInternalDataStructures(QPointer <Player> player);

    void ProcessNewAssets();

    QList <QPointer <RoomObject> > DoRoomLoad(QHash <QString, QPointer <RoomObject> > & envobjects, QPointer <Player> player);
    QList <QPointer <RoomObject> > DoRoomUpdate(QHash <QString, QPointer <RoomObject> > & envobjects, QPointer <Player> player, const QScriptValueList & args);
