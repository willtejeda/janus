    ...
    delta_time(0.0f)
{
    ...
    focus_websurface_cursor_uv = QPointF(0,0);
    websurface_focused = false;

    pinching = false;

    last_screen_rotation = JNIUtil::GetScreenOrientation();

    gyroscope = new QGyroscope(this);
    gyroscope->start();
    gyroscope_timer.start();

    internet_connected = true;
    remove_headset = false;
#endif
...
#else
    teleport_hold_time_required = 375;
#endif
...
#ifdef __ANDROID__
    network_checker = new QNetworkAccessManager();
    connect(network_checker, SIGNAL(finished(QNetworkReply*)), this, SLOT(CheckInternetConnection(QNetworkReply*)));
...
void Game::RequestInternetConnection()
{
    //qDebug() << "REQ";
    QNetworkRequest req(QUrl("http://www.google.com"));
    network_checker->get(req);
}
#endif
...
{
    for (int i=0; i<private_websurfaces.size(); ++i) {
        QPointer <AbstractWebSurface> a = private_websurfaces[i].asset;
        if (a) {
            if (!a->GetStarted()) {
                a->SetStarted(true);
                a->Load();
...
void Game::Update()
{
    ...
    if (!do_exit) {
        //REFACTORTODO - update any assets in here, call "Update"
    }
...
//        qDebug() << "alpha" << alpha << MathUtil::ModelMatrix() << MathUtil::ViewMatrix();
        ...
        AbstractRenderCommand a(PrimitiveType::TRIANGLES,
                                ...
                                0,
                                0,
                                renderer->GetTexturedCube2VAO().get(),
...
void Game::initializeGL()
{
    ...
    //textgeom
    //qDebug() << "Game::initializeGL() - Initializing text...";
    TextGeom::initializeGL();

    //load buttons
    //qDebug() << "Game::initializeGL() - Initializing buttons...";
    RoomObject::initializeGL();

    //qDebug() << "Game::initializeGL() - Initializing loading animations...";
...
#ifdef __ANDROID__
void Game::mouseMoveEvent(QMouseEvent * e, const float x, const float y, const int cursor_index, const QSize p_windowSize, const QPointF p_mousePos)
#else
...
#else
        mouse_move_distance += sqrt(pow(x, 2) + pow(y, 2));
        if (!websurface_focused && (mouse_move_distance > qMin(win_size.width(),win_size.height())/6) && (mouse_move_time.elapsed() > 250)){
            player->SpinView(x * 0.1f * (SettingsManager::GetInvertXEnabled() ? -1.0f : 1.0f), true);
            if (GetMouseDoPitch()) {
                const float tilt_amount = y * 0.1f * (SettingsManager::GetInvertYEnabled() ? 1.0f : -1.0f);
                player->TiltView(tilt_amount);
            }
...
//        qDebug() << "Game::mouseMoveEvent" << selected[cursor_index] << envobjects.contains(selected[cursor_index]);
    ...
    //Need to compute cursor raycast right away for mouse press
        if (!controller_manager->GetUsingSpatiallyTrackedControllers()) {
            ComputeMouseCursorTransform(p_windowSize, p_mousePos);
            UpdateCursorRaycast(GetMouseCursorTransform(), 0);
        }

        if (websurface_focused && websurface_selected[cursor_index]){

            //Drag and drop if pressed for long enough
            if ((mouse_move_distance < qMin(win_size.width(),win_size.height())/6) && mouse_move_time.elapsed() > 1000) {

                DragAndDropFromWebsurface(keys[Qt::Key_Control]?"Drag+Drop":"Drag+Pin", cursor_index);

                mouse_move_distance = 0.0f;
            ...
            focus_websurface_cursor_uv = cursor_uv[cursor_index];

            QWheelEvent e3(cursor_pos, cursor_pos.y() - last_scroll.y(), e->buttons(), e->modifiers());
            //qDebug() << "YOFFSET" << cursor_pos.y() - last_scroll.y();
            QWheelEvent e4(cursor_pos, cursor_pos.x() - last_scroll.x(), e->buttons(), e->modifiers(), Qt::Horizontal);
            //qDebug() << "XOFFSET" << cursor_pos.x() - last_scroll.x();
            websurface_selected[cursor_index]->wheelEvent(&e3);
            websurface_selected[cursor_index]->wheelEvent(&e4);
            last_scroll = cursor_pos;
        }
#else
...
#ifdef __ANDROID__
void Game::mousePressEvent(QMouseEvent * e, const int cursor_index, const QSize p_windowSize, const QPointF p_mousePos)
#else
...
#endif
{
    ...
    case JVR_STATE_DEFAULT:
        if (e->button() == Qt::LeftButton) {
            ...
            if (o && o->GetType() == "object" && o->GetAssetWebSurface()) {
...
#ifdef __ANDROID__
void Game::mouseReleaseEvent(QMouseEvent * e, const int cursor_index, const QSize p_windowSize, const QPointF p_mousePos)
#else
...
#endif
{
    ...
    case JVR_STATE_DEFAULT:
    {
        if (e->button() == Qt::LeftButton) {
            ...
            //Prevent unintended click behaviour (e.g. open portals) when releasing after a mouse move
            if ((mouse_move_distance > qMin(win_size.width(),win_size.height())/6) && (mouse_move_time.elapsed() > 250)){
                mouse_move_distance = 0.0f;
...
//    qDebug() << "Game::keyReleaseEvent" << e->key();
    ...
    {
        MathUtil::m_loop_render = !MathUtil::m_loop_render;
        qDebug() << "m_do_updates = " << MathUtil::m_loop_render;
    }*/
...
//    if (state == JVR_STATE_DEFAULT && (web_sel.isNull() || web_sel == menu.GetWebpage())) {
//        menu.SetCurrentKey(QString());
//    }
    ...
    if (state == JVR_STATE_UNIT_TRANSLATE ||
        ...
        for (int i=0; i<conn_list.size(); ++i) {
            ...
            for (int j=0; j<s.rooms.size(); ++j) {
                if (cur_url == s.rooms[j].url) {
                    info2_text_geom.AddText("*" + s.rooms[j].url, QColor(178,178,255));
                }
                else if (s.rooms[j].sent_subscribe) {
                    info2_text_geom.AddText(" " + s.rooms[j].url, QColor(102, 102, 255));
                }
                else {
                    info2_text_geom.AddText(" " + s.rooms[j].url, QColor(51,51,255));
                }
            }
        }
    }
...
void Game::DrawOverlaysGL()
{
...
#ifdef __ANDROID__
    //Draw an infographic telling user to remove headset to accept permissions
    if (RoomObject::remove_headset_img && remove_headset){
        MathUtil::PushModelMatrix();
        MathUtil::MultModelMatrix(m);
        MathUtil::ModelMatrix().scale(1,RoomObject::remove_headset_img->GetAspectRatio(),1);
        shader->UpdateObjectUniforms();

        RendererInterface::m_pimpl->BindTextureHandle(0, RoomObject::remove_headset_img->GetTextureHandle(true));
        shader->SetUseTexture(0, true);
    ...
    //Draw an infographic if user is not connected to internet
    else if (RoomObject::internet_connected_img && !internet_connected){
        MathUtil::PushModelMatrix();
        MathUtil::MultModelMatrix(m);
        MathUtil::ModelMatrix().scale(1,RoomObject::internet_connected_img->GetAspectRatio(),1);
        shader->UpdateObjectUniforms();

        RendererInterface::m_pimpl->BindTextureHandle(0, RoomObject::internet_connected_img->GetTextureHandle(true));
        shader->SetUseTexture(0, true);
        SpinAnimation::DrawPlaneGL(shader, QColor(255,255,255,255));
        shader->SetUseTexture(0, false);
    ...
    //Draw loading icon as overlay if room is not loaded
        ...
        shader->UpdateObjectUniforms();
        const float value = r->GetProgress();
        SpinAnimation::DrawGL(shader, value, false);
        MathUtil::PopModelMatrix();
    }
#endif
...
QPointer <AbstractWebSurface> Game::GetWebSurfaceSelected()
{
    if (websurface_selected[0]) {
    ...
    if (controller_manager->GetUsingSpatiallyTrackedControllers()
        ...
        if (player->GetS("hmd_type") == "rift") {
            ...
            if (c) {
                ...
                c->setProperty("left_trigger", s[0].t[0].value);
                c->setProperty("left_trackpad_click", s[0].b[0].value);
                c->setProperty("left_menu", s[0].b[1].value); //App button

                // For Gear this will be the headset controls
                c->setProperty("right_trackpad_x", s[1].x);
                c->setProperty("right_trackpad_y", s[1].y);
                c->setProperty("right_trigger", s[1].t[0].value);
                c->setProperty("right_trackpad_click", s[1].b[0].value);
                c->setProperty("right_menu", s[1].b[1].value); //App button
            }
        }
#endif
    }
...
//            qDebug() << "GAME::UPDATECONTROLLERS B_POCKETSPACE" << b_pocketspace.hover << b_pocketspace.pressed << b_pocketspace.proc_press << b_pocketspace.proc_release;
            ...
            if (r->GetRoomObject(player->GetCursorObject(i))) {
                s[i].laser_angle = 0.0f; //Always draw laser if user is pointing at something
            }
            else{
                s[i].laser_angle = angle;
            }
            //qDebug() << "laser_angle" << MathUtil::RadToDeg(asin(QVector3D::dotProduct(controller_dir, QVector3D(0.0f,1.0f,0.0f)))) << angle << s[i].laser_angle;
#else
            if (b_click.hover || b_teleport.hover || b_grab.pressed) {
                dist = UpdateCursorRaycast(m2, i);
            }
            ...
            if (state == JVR_STATE_DEFAULT || state == JVR_STATE_INTERACT_TELEPORT) {
...
#ifdef __ANDROID__
                if (b_click.proc_press && (sqrt(pow(x0,2) + pow(y0,2)) > axis_threshold)){
                    websurface_focused = false;
                    focus_websurface.clear();
                    focus_websurface_cursor_uv = QPointF(0,0);
...
#ifdef __ANDROID__
                if (b_click.proc_release && (sqrt(pow(x0,2) + pow(y0,2)) > axis_threshold)){
                ...
                if (b_click.proc_release) {
                    ...
                    if (!room_has_fn_onmouseup && !room_has_fn_onclick) {
                        ...
                        if (!(state == JVR_STATE_INTERACT_TELEPORT && teleport_held_time.elapsed() > teleport_hold_time_required))
#endif
...
//    }
...
#ifdef __ANDROID__
    if (controller_manager->GetHMDManager() && controller_manager->GetHMDManager()->GetEnabled() && controller_manager->GetUsingSpatiallyTrackedControllers() && websurface_selected[0]){
        if (fabsf(controller_y[0]) > snapturn_axis_threshold) {
            QWheelEvent e2(QPointF(), controller_y[0] * 50.0f, 0, 0);
            websurface_selected[0]->wheelEvent(&e2);
        }
        if (fabsf(controller_x[0]) > snapturn_axis_threshold) {
            QWheelEvent e3(QPointF(), controller_x[0] * 50.0f, 0, 0, Qt::Horizontal);
    ...
    if (SettingsManager::GetUseGyroscope())
    {
        QGyroscopeReading * r = gyroscope->reading();
        int screen_rotation = last_screen_rotation;
        last_screen_rotation = JNIUtil::GetScreenOrientation();

        //Reset tilt if phone orientation has changed
        if (screen_rotation == last_screen_rotation){
            float time = gyroscope_timer.elapsed() / 1000.0f;

            if (screen_rotation == 0){
                player->TiltView(-r->x() * time );
                player->SpinView(-r->y() * time, false);
            }
            else if (screen_rotation == 90){
                player->TiltView(r->y() * time );
                player->SpinView(-r->x() * time, false);
            }
            else if (screen_rotation == 180){
                player->TiltView(r->x() * time );
                player->SpinView(r->y() * time, false);
            }
            else if (screen_rotation == 270){
                player->TiltView(-r->y() * time );
                player->SpinView(r->x() * time, false);
            }
        }
...
#ifdef __ANDROID__
bool Game::GetPinching()
{
    return pinching;
}

void Game::SetPinching(const bool b)
{
    pinching = b;
}
#endif
