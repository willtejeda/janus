    ...
    m_color_mask(p_copy.m_color_mask)
{
    ...
    for (int i = 0; i < ASSETSHADER_NUM_COMBINED_TEXURES; ++i)
    {
        if (m_material_uniforms.iUseTexture[i] != 0.0)
        {
            ...
            {
                if (m_texture_set.GetTextureHandleRef(i, true) != p_copy.m_texture_set.GetTextureHandleRef(i, true))
                {
            ...
            else
            {
                ...
                if (m_texture_set.GetTextureHandleRef(i, false) != p_copy.m_texture_set.GetTextureHandleRef(i, false))
                {
...
ProgramHandle* AbstractRenderCommand::GetShaderRef()
{
    return m_shader;
}

void AbstractRenderCommand::SetShader(ProgramHandle * p_shader)
{
    m_shader = p_shader;
}
...
int TextureSet::ContainsTextureHandle(TextureHandle *p_id) const
{
    int index = -1;
    for (int i = 0; i < ASSETSHADER_NUM_COMBINED_TEXURES; ++i)
    {
        if (m_texture_handles_left[i] == p_id || m_texture_handles_right[i] == p_id)
        {
            index = i;
            break;
        }
