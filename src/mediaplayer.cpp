...
TextureHandle* MediaPlayer::GetTextureHandle(MediaContext * ctx)
{
    UpdateTexture(ctx);
    return ctx->m_texture_handles[0].get();
}

TextureHandle* MediaPlayer::GetLeftTextureHandle(MediaContext * ctx)
{
    UpdateLeftRightTextures(ctx);
    return ctx->m_texture_handles[0].get();
}

TextureHandle* MediaPlayer::GetRightTextureHandle(MediaContext * ctx)
{
    UpdateLeftRightTextures(ctx);
    return ctx->m_texture_handles[1].get();
}
