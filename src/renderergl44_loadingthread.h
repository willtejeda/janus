...
public:
    ...
    void CompileAndLinkShaderProgram2(std::shared_ptr<ProgramHandle> *p_abstract_program, QByteArray *p_vertex_shader, QString p_vertex_shader_path, QByteArray *p_fragment_shader, QString p_fragment_shader_path, std::vector<std::vector<GLint> > *p_map);
...
signals:
    ...
    void Process(AbstractRenderer *, std::unordered_map<size_t, std::vector<AbstractRenderCommand>> *, std::unordered_map<StencilReferenceValue, LightContainer> *);
private:
