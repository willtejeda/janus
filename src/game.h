...
{
    QPointer <AbstractWebSurface> asset;    
...
public:
    ...
    QPointer <AbstractWebSurface> GetWebSurfaceSelected();   
    ...
    void SetRemoveHeadset(const bool b);
...
private:   
    ...
    QPointer <AbstractWebSurface> websurface_selected[2];    
    ...
    QTime gyroscope_timer;

    float mouse_move_distance;
    QTime mouse_move_time;

    bool websurface_focused;
    QPointF last_scroll;
    QPointer <AbstractWebSurface> focus_websurface;
    QPointF focus_websurface_cursor_uv;

    bool pinching;

    int last_screen_rotation;

    QNetworkAccessManager * network_checker;
    QTime network_timer;
