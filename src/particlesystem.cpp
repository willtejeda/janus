...
/*#ifdef __ANDROID__
                    m_positions[position_offset + 0] = half_float::half(0.0f);
                    m_positions[position_offset + 1] = half_float::half(0.0f);
                    m_positions[position_offset + 2] = half_float::half(0.0f);
                    m_positions[position_offset + 3] = half_float::half(1.0f);
#else*/
...
/*#ifdef __ANDROID__
                uint32_t position_offset = vertex_final_index * elements_per_position;
                m_positions[position_offset + 0] = half_float::half(v[vertex_index].x());
                m_positions[position_offset + 1] = half_float::half(v[vertex_index].y());
                m_positions[position_offset + 2] = half_float::half(v[vertex_index].z());
                m_positions[position_offset + 3] = half_float::half(1.0f);

                uint32_t const color_offset = vertex_final_index * elements_per_color;
                m_colors[color_offset + 0] = half_float::half(premultipled_red);
                m_colors[color_offset + 1] = half_float::half(premultipled_green);
                m_colors[color_offset + 2] = half_float::half(premultipled_blue);
                m_colors[color_offset + 3] = half_float::half(premultipled_alpha);
#else*/
                uint32_t position_offset = vertex_final_index * elements_per_position;
...
//        qDebug() << "p_emitted" << p_emitted;
    ...
    {
/*#ifdef __ANDROID__
        RendererInterface::m_pimpl->UpdateBufferHandleData(m_VBO_positions, 0, static_cast<uint32_t>(m_positions.size() * sizeof(half_float::half)), m_positions.data());
        RendererInterface::m_pimpl->UpdateBufferHandleData(m_VBO_colors, 0, static_cast<uint32_t>(m_colors.size() * sizeof(half_float::half)), m_colors.data());
#else*/
...
/*#ifdef __ANDROID__
            ...
            uint32_t const color_offset = vertex_final_index * elements_per_color;
            m_colors[color_offset + 0] = half_float::half(1.0f);
            m_colors[color_offset + 1] = half_float::half(1.0f);
            m_colors[color_offset + 2] = half_float::half(1.0f);
            m_colors[color_offset + 3] = half_float::half(1.0f);
#else*/
            uint32_t position_offset = vertex_final_index * elements_per_position;
            m_positions[position_offset + 0] = float(0.0f);
            m_positions[position_offset + 1] = float(0.0f);
...
//#endif
    ...
    //RendererInterface::m_pimpl->BindMeshHandle(m_mesh_handle.get());

    auto buffers = RendererInterface::m_pimpl->GetBufferHandlesForMeshHandle(m_mesh_handle.get());

    m_VBO_positions = (*buffers)[(GLuint)VAO_ATTRIB::POSITION];
    if (m_VBO_positions) {
        RendererInterface::m_pimpl->BindBufferHandle(m_VBO_positions.get());
    ...
    if (m_VBO_tex_coords0) {
        RendererInterface::m_pimpl->BindBufferHandle(m_VBO_tex_coords0.get());
    ...
    if (m_VBO_colors) {
        RendererInterface::m_pimpl->BindBufferHandle(m_VBO_colors.get());
    ...
    if (m_VBO_indices) {
        RendererInterface::m_pimpl->BindBufferHandle(m_VBO_indices.get());
